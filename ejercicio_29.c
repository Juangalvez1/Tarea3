...
- Un descriptivo de que hace el programa:
    Este programa calcula la serie de tailor de e^x, x y la cantidad de terminos de la serie de taylor lo ingresa el usuario.
- Salvedades:
  Para pruebas hechas en otros sistemas operativos y/o otras versiones del lenguaje, no se garantizan resultados
  Para valores ingresados que no sean numeros reales para la x, no se garantizan resultados.
...
double CalculateExponential(double x, int times, int index){
    if (index > times){
        return 0;
    } else {
        return pow(x, index) / CalculateFactorial(index) + CalculateExponential(x, times, index + 1);
    }
...
int main(){
    ...
    printf("Este programa calcula por serie de taylor la funcion e^x."); //Explicamos brevemente lo que hace el programa
    ...
    printf("\nEl resultado es de: %lf", CalculateExponential(x, terms, 0));
