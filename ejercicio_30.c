...
double CalculateCos(double x, int times, int index){
    if (index > times){
        return 0;
    } else {
        return (pow(-1, index) * pow(x, 2 * index))/ CalculateFactorial(2 * index) + CalculateCos(x, times, index + 1);
    }
...
int main(){
    ...
    printf("Este programa calcula por serie de taylor la funcion cosx."); //Explicamos brevemente lo que hace el programa
    ...
    printf("\nEl resultado es de: %lf", CalculateCos(x, terms, 0));
