...
- Un descriptivo de que hace el programa:
    Este programa calcula la serie de tailor de Cosx, x y la cantidad de terminos de la serie de taylor lo ingresa el usuario.
- Salvedades:
...
double CalculateSenh(double x, int times, int index){
    if (index > times){
        return 0;
    } else {
        return pow(x, 2 * index + 1)/ CalculateFactorial(2 * index + 1) + CalculateSenh(x, times, index + 1);
    }
}
...
int main(){
    ...
    printf("Este programa calcula por serie de taylor la funcion senhx."); //Explicamos brevemente lo que hace el programa
    ...
    printf("\nEl resultado es de: %lf", CalculateSenh(x, terms, 0));
