...
- Un descriptivo de que hace el programa:
    Este programa calcula la serie de tailor de coshx, x y la cantidad de terminos de la serie de taylor lo ingresa el usuario.
- Salvedades:
...
double CalculateCosh(double x, int times, int index){
    if (index > times){
        return 0;
    } else {
        return pow(x, 2 * index)/ CalculateFactorial(2 * index) + CalculateCosh(x, times, index + 1);
    }
}
...
int main(){
    ...
    printf("Este programa calcula por serie de taylor la funcion coshx."); //Explicamos brevemente lo que hace el programa
    ...
    printf("\nEl resultado es de: %lf", CalculateCosh(x, terms, 0));
