...
- Un descriptivo de que hace el programa:
    Este programa calcula la serie de tailor de lnx, x y la cantidad de terminos de la serie de taylor lo ingresa el usuario.
- Salvedades:
...
double CalculateLn(double x, int times, int index){
    if (index > times){
        return 0;
    } else {
        return (pow(-1 , index + 1) * pow(x - 1, index)) / index + CalculateLn(x, times, index + 1);
    }
}
...
int main(){
    ...
    printf("Este programa calcula por serie de taylor la funcion lnx."); //Explicamos brevemente lo que hace el programa
    ...
    printf("\nEl resultado es de: %lf", CalculateLn(x, terms, 1));
