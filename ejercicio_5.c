...
- Un descriptivo de que hace el programa:
    Este programa muestra los n-primeros terminos de la serie de Perrin, n es ingresado por el usuario
- Salvedades:
...
void ShowPerrinSeries(int number1, int number2, int number3, int index, int times){ //Se crea la funcion ShoePerrinSeries en la que se ira calculando e imprimiendo el siguiente numero de la serie de perrin
    if(index <= times){
        if (index == times){
            ...
            printf("%i, ", number1);
            ShowPerrinSeries(number2, number3, number1+number2, index+1, times); //Hace el llamado recursivo
        }
        return;
    }
}
...
int main() {
    ...
    printf("Este programa presenta los n primeros numeros de la serie de Perrin\nIngrese el numero de terminos que quiere ver: "); //Se explica lo que hara el programa
    scanf("%i", &times);
    ShowPerrinSeries(3, 0, 2, 1, times); //Se hace el llamado a la funcion que imprime la serie de perrin
