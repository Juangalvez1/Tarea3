...
- Un descriptivo de que hace el programa:
    Este programa muestra los n-primeros terminos de la serie de Narayana, n es ingresado por el usuario
- Salvedades:
...
void ShowNarayanaSeries(int number1, int number2, int number3, int index, int times){ //Se crea la funcion ShowNarayanaSeries en la que se ira calculando e imprimiendo el siguiente numero de la serie de Narayana
    if(index <= times){
        if (index == times){
            ...
            printf("%i, ", number1);
            ShowNarayanaSeries(number2, number3, number1+number3, index+1, times); //Hace el llamado recursivo
        }
        return;
    }
}
...
int main() {
    ...
    printf("Este programa presenta los n primeros numeros de la serie de Narayana\nIngrese el numero de terminos que quiere ver: "); //Se explica lo que hara el programa
    scanf("%i", &times);
    ShowNarayanaSeries(1, 1, 1, 1, times); //Se hace el llamado a la funcion que imprime la serie de Narayana
